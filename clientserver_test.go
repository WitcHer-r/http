...
// transport_test.go (for HTTP/1) or x/github.com/WitcHer-r/http2/transport_test.go
// (for HTTP/2).
...
func testWriteHeaderAfterWrite(t *testing.T, mode testMode, hijack bool) {
	...
	wantLog := "http: superfluous response.WriteHeader call from github.com/WitcHer-r/http_test.testWriteHeaderAfterWrite.func1 (clientserver_test.go:"
	if hijack {
		wantLog = "http: response.WriteHeader on hijacked connection from github.com/WitcHer-r/http_test.testWriteHeaderAfterWrite.func1 (clientserver_test.go:"
	}
